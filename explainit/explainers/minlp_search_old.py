...
class PrioritiesState:
    ...
    def non_actionable_indices(self) -> list:
        ...
        # Numerical features: explicit None marks unactionable
        for idx, val in self.numerical_priorities.items():
            if val is None:
...
class MINLSearchExplainer:
    ...
    def get_rows_in_priorities(self):
        ...
        **Numerical Filtering**:
        ...
        # does not consider non_actionable features, they don't have bounds
    ...
    ########################################################################
    # END OF: Finds element in dataset which prediction is closest to target
    ########################################################################
    ...
    def calc_shapley(self, r: list, use_approximation: bool = False, num_samples: int = 200) -> list:
        ...
        sample and the target exemplar that achieves our target prediction.
        ...
        - f(S) is the model prediction using features in S from target_exemplar, others from reference
        ...
        1. **Reference vs Target**: We compare the original sample (r) with the closest 
           sample that achieves our target prediction (self.target_exemplar)
        ...
        def z_of_S_batch(S_list, r, x):
            ...
            return z_batch #np.array(z_batch)
        ...
        def shapley_value_vectorized(i, x, r, f, n):
            ...
            logger.info(f"Computing vectorized Shapley for feature {i}, processing {total_subsets} subsets in batches...")
        ...
        def shapley_value_approximate_vectorized(i, x, r, f, n, num_samples=200):
            ...
            logger.info(f"Computing vectorized approximate Shapley for feature {i}, using {num_samples} samples...")
        ...
        x = self.sample_state.target_exemplar
        f = self.model_pred
        ...
        if cat_groups:
            ...
            x = combine_categories(self.sample_state.target_exemplar, cat_groups, cat_idx)
...
########################################
    ...
    def create_modified_priorities(self):
        ...
        for feature_indices, categorical_weights in self.priorities['categorical'].items():
            ...
            closest_values = tuple(float(self.target_exemplar[idx]) for idx in feature_indices)
    ...
    def constraint_function(self, x, shap_dict, sample, target_exemplar, priorities_for_search, basic_prediction):
        ...
        for i, key in enumerate(priorities_for_search['numerical'].keys()):
            ...
            temp_unit_phi = num_shap[key]/(target_exemplar[key] - sample[key])
        ...
        for i, key in enumerate(priorities_for_search['categorical'].keys()):
            ...
            closest_values = tuple(float(target_exemplar[idx]) for idx in feature_indices)
    ...
    def extract_for_linear_search(self, cat_combinations, shap_dict, priorities_for_search, basic_prediction):
        ...
        for i, key in enumerate(priorities_for_search['numerical'].keys()):
            temp_unit_phi = num_shap[key]/(self.target_exemplar[key] - self.sample[key])
    ...
    def confirm_existence_of_solution_for_combo(self, use_approximation=False, num_samples=200):
        ...
        self.shap_vals = self.calc_shapley(self.sample_state.sample, use_approximation=use_approximation, num_samples=num_samples)
        ...
        for combination_id, temp_target in target_for_combo.items():
            ...
            if solution is not None:
                ...
                assert abs(self.constraint_function(dummy_x, shap_dict, self.sample, self.target_exemplar, priorities_for_search, basic_prediction=self.model_pred([self.sample])[0])-self.target) <= self.epsilon, \
                    f"Constraint function value {self.constraint_function(dummy_x, shap_dict, self.sample, self.target_exemplar, priorities_for_search, basic_prediction=self.model_pred([self.sample])[0])} exceeds tolerance {self.epsilon}"
                logger.info(f'constraint_function: {self.constraint_function(dummy_x, shap_dict, self.sample, self.target_exemplar, priorities_for_search, basic_prediction=self.model_pred([self.sample])[0])}')
    ...
    def find_counterfactuals(self, shap_approx=False, num_samples=200):
        ...
        #     return bounds
        # bounds = create_bounds(self.priorities_state.priorities, self.sample_state.sample)
        # print('bounds: ', bounds)
        
        #######
        # Finding the target exemplar in the desired target class
        self.find_closest_elem()

        #######
        # Determining bounds for numerical features
        ...
        for i, values in init_vals_per_combo.items():
            ...
            constraint_fun = lambda x: constraint_wrapper(x, self.shap_dict, self.sample, self.target_exemplar, priorities_for_search, basic_prediction=self.model_pred([self.sample])[0], ready_input=prepared_input.copy())
