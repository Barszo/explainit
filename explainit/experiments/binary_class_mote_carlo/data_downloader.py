...
def _download_credit_card_default(raw_file):
    ...
    # Strategy 3: direct ZIP + XLS download from UCI
    try:
        import zipfile
        import io
        ...
        response = _req.get(zip_url, timeout=120)
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as z:
            ...
            with z.open(xls_name) as xls_bytes:
                df = pd.read_excel(io.BytesIO(xls_bytes.read()), header=1)
        df.to_csv(raw_file, index=False)
        print(f"✓ Downloaded from UCI (XLS), saved to: {raw_file}")
        return df
    except ImportError:
        print("  openpyxl not installed (needed for XLS). Run: pip install openpyxl")
    except Exception as e:
        print(f"  UCI direct download failed: {e}")
