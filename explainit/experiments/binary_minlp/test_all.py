...
def get_standard_cf_methods(config, model, X_train, y_train, feature_names, feature_bounds=None,
                            categorical_feature_names=None, target_sample=None):
    """
    ...
    Args:
        ...
        target_sample: Optional target exemplar used by methods that need per-sample
            preferences (e.g., MINLP).
    ...
    for method_key in selected_method_keys:
        if method_key == 'wachter':
        ...
        elif method_key == 'minlp':
            if target_sample is None:
                logger.warning(
                    "MINLP method requested but no target_sample provided; skipping."
                )
                continue

            minlp_cfg = config['standard_methods'].get('minlp', {})
            exemplar_weight = minlp_cfg.get(
                'exemplar_weight',
                config['preference_method'].get('exemplar_weight', 0.85)
            )
            steepness = minlp_cfg.get(
                'preference_steepness',
                config['preference_method'].get('preference_steepness', 5)
            )
            target_probability = minlp_cfg.get(
                'target_probability',
                config['preference_method'].get('target_probability', 0.75)
            )
            threshold = minlp_cfg.get(
                'threshold',
                config['samples'].get('threshold', 0.5)
            )

            captured_target_sample = np.array(target_sample, dtype=float).flatten()
            captured_X_train = X_train
            captured_feature_names = feature_names

            def _build_preferences_for_sample(sample,
                                              _ts=captured_target_sample,
                                              _xt=captured_X_train,
                                              _fn=captured_feature_names,
                                              _ew=exemplar_weight,
                                              _st=steepness):
                preferences, _ = define_preferences(
                    sample=sample,
                    target_sample=_ts,
                    X_train=_xt,
                    feature_names=_fn,
            ...
            methods['MINLP Search'] = MinlpCounterfactual(
                model=model,
                X_train=X_train,
                y_train=y_train,
                feature_names=feature_names,
                preferences_builder=_build_preferences_for_sample,
                num_cfs=num_cfs,
                max_iterations=minlp_cfg.get('max_iterations', max_iter),
                target_class=target_class,
                threshold=threshold,
                target_probability=target_probability,
                target_exemplar_epsilon=minlp_cfg.get('target_exemplar_epsilon', 0.05),
                epsilon=minlp_cfg.get('epsilon', 0.1),
                shap_approx=minlp_cfg.get('shap_approx', True),
                shap_num_samples=minlp_cfg.get('shap_num_samples', 200),
                feature_bounds=feature_bounds,
            )
